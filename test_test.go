...
func TestCreateTasks(t *testing.T) {
	...
	t.Run("should return an error if name is empty", func(t *testing.T) {
		actualTask := &Task{Name: ""}

		b, err := json.Marshal(actualTask)
		if err != nil {
			t.Fatal(err)
		}
...
func TestGetTask(t *testing.T) {
	...
	t.Run("should return the task if exist otherwise not found", func(t *testing.T) {
		req, err := http.NewRequest(http.MethodGet, "/tasks/99", nil)
		if err != nil {
			t.Fatal(err)
		}
